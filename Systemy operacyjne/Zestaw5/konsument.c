...
int main (int argc, char *argv[]) {
	...
	int potok;
	int odczyt;
	const size_t rozmiarDanychKonsument = 20; // nie może być większy niż 1024
	const double maxDlugoscCzekania = 2.5;
	
	// sprawdzenie poprawności argumentów
	if (argc != 3) {
		printf ("Błąd: niepoprawna liczba argumentów!\n");
		printf ("Wywołaj program wpisując: %s plik_wyjsciowy potok\n", argv[0]);
		exit (EXIT_FAILURE);
	}
	...
	if ((plik = open (argv[1], O_WRONLY | O_CREAT | O_TRUNC, 0644)) == -1) {
		printf ("Błąd otwarcia pliku %s do zapisu\n", argv[1]);
		perror ("Błąd pliku");
		exit (EXIT_FAILURE);
	}
